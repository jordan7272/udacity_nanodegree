import media
import fresh_tomatoes

ADVENTURELAND = media.Movie("Adventureland",
                            "https://upload.wikimedia.org/wikipedia/en/"
                            "thumb/b/bb/Adventurelandposter.jpg/"
                            "215px-Adventurelandposter.jpg",
                            "https://www.youtube.com/watch?v=gfPE_MEKipM",
                            "Greg Mottola",
                            "Jesse Eisenberg, Kristen Stewart, Ryan Reynolds",
                            "2009", False, None)


ALMOST_FAMOUS = media.Movie("Almost Famous",
                            "https://upload.wikimedia.org/wikipedia/en/"
                            "thumb/d/dd/Almost_famous_poster1.jpg/"
                            "220px-Almost_famous_poster1.jpg",
                            "https://www.youtube.com/watch?v=6iyp0qcf7-w",
                            "Cameron Crowe",
                            "Billy Crudup, Patrick Fugit, Kate Hudson",
                            "2000", False, None)


AMERICAN_HUSTLE = media.Movie("American Hustle",
                              "https://upload.wikimedia.org/wikipedia/en/"
                              "thumb/8/85/American_Hustle_2013_poster.jpg/"
                              "220px-American_Hustle_2013_poster.jpg",
                              "https://www.youtube.com/watch?v=ST7a1aK_lG0",
                              "David O. Russell",
                              "Christian Bale, Amy Adams, Bradley Cooper",
                              "2013", False, None)

AMERICAN_PSYCHO = media.Movie("American Psycho",
                              "https://upload.wikimedia.org/wikipedia/en/"
                              "thumb/6/63/Americanpsychoposter.jpg/"
                              "220px-Americanpsychoposter.jpg",
                              "https://www.youtube.com/watch?v=2GIsExb5jJU",
                              "Mary Harron",
                              "Christian Bale, Justin Theroux, Josh Lucas",
                              "2000", False, None)

BIG_LEBOWSKI = media.Movie("Big Lebowski",
                           "https://upload.wikimedia.org/wikipedia/en/"
                           "thumb/3/35/Biglebowskiposter.jpg/"
                           "220px-Biglebowskiposter.jpg",
                           "https://www.youtube.com/watch?v=cd-go0oBF4Y",
                           "Joel Coen, Ethan Coen",
                           "Jeff Bridges, John Goodman, Julianne Moore",
                           "1998", False, None)

BIRDMAN = media.Movie("Birdman",
                      "https://upload.wikimedia.org/wikipedia/en/"
                      "thumb/a/a3/Birdman_poster.jpg/"
                      "220px-Birdman_poster.jpg",
                      "https://www.youtube.com/watch?v=uJfLoE6hanc",
                      "Alejandro Gonzalez Inarritu",
                      "Michael Keaton, Zach Galifianakis, Edward Norton",
                      "2014", False, None)

BROKEN_FLOWERS = media.Movie("Broken Flowers",
                             "https://upload.wikimedia.org/wikipedia/en/"
                             "thumb/7/79/Broken_Flowers_poster.jpg/"
                             "220px-Broken_Flowers_poster.jpg",
                             "https://www.youtube.com/watch?v=c_TB7MkrGyc",
                             "Jim Jarmusch",
                             "Bill Murray, Jessica Lange, Sharon Stone",
                             "2005", False, None)

DARJEELING_LIMITED = media.Movie("Darjeeling Limited",
                                 "https://upload.wikimedia.org/wikipedia/en/"
                                 "thumb/1/1e/Darjeeling_Limited_Poster.jpg/"
                                 "220px-Darjeeling_Limited_Poster.jpg",
                                 "https://www.youtube.com/watch?v=aO1bYukdvLI",
                                 "Wes Anderson",
                                 "Owen Wilson, Adrien Brody,\
                                  Jason Schwartzman",
                                 "2007", True, None)

EDWARD_SCISSORHANDS = media.Movie("Edward Scissorhands",
                                  "https://upload.wikimedia.org/wikipedia/en/"
                                  "thumb/3/3b/Edwardscissorhandsposter.JPG/"
                                  "220px-Edwardscissorhandsposter.JPG",
                                  "https://www.youtube.com/"
                                  "watch?v=M94yyfWy-KI",
                                  "Tim Burton",
                                  "Johnny Depp, Winona Ryder, Dianne Wiest",
                                  "1990", False, None)

EYES_WIDE_SHUT = media.Movie("Eyes Wide Shut",
                             "https://upload.wikimedia.org/wikipedia/en/"
                             "thumb/c/c3/Ghostworldposter.jpg/"
                             "215px-Ghostworldposter.jpg",
                             "https://www.youtube.com/watch?v=YEfyfcEdW4Y",
                             "Stanley Kubrick",
                             "Tom Cruise, Nicole Kidman, Todd Field",
                             "1999", False, None)

GHOST_WORLD = media.Movie("Ghost World",
                          "https://upload.wikimedia.org/wikipedia/en/"
                          "thumb/8/87/Eyes_Wide_Shut_poster.jpg/"
                          "220px-Eyes_Wide_Shut_poster.jpg",
                          "https://www.youtube.com/watch?v=YEfyfcEdW4Y",
                          "Terry Zwigoff",
                          "Steve Buscemi, Thora Birch, Scarlett Johansson",
                          "2001", False, None)

GOODFELLAS = media.Movie("Goodfellas",
                         "https://upload.wikimedia.org/wikipedia/en/"
                         "thumb/7/7b/Goodfellas.jpg/"
                         "220px-Goodfellas.jpg",
                         "https://www.youtube.com/watch?v=2ilzidi_J8Q",
                         "Martin Scorsese",
                         "Robert De Niro, Ray Liotta, Joe Pesci",
                         "1990", False, None)

GRAND_BUDAPEST_HOTEL = media.Movie("Grand Budapest Hotel",
                                   "https://upload.wikimedia.org/wikipedia/en/"
                                   "thumb/a/a6/"
                                   "The_Grand_Budapest_Hotel_Poster.jpg/"
                                   "220px-The_Grand_Budapest_Hotel_Poster.jpg",
                                   "https://www.youtube.com/"
                                   "watch?v=1Fg5iWmQjwk",
                                   "Wes Anderson",
                                   "Ralph Fiennes, F. Murray Abraham,\
                                    Mathieu Amalric",
                                   "2014", False, None)

HOLY_MOUNTAIN = media.Movie("Holy Mountain",
                            "https://upload.wikimedia.org/wikipedia/en/"
                            "thumb/3/39/Holy_Mountain.gif/"
                            "220px-Holy_Mountain.gif",
                            "https://www.youtube.com/watch?v=V_k8oaeHsnc",
                            "Alejandro Jodorowsky",
                            "Alejandro Jodorowsky, Horacio Salinas, Zamira",
                            "1973", False, None)

HOT_FUZZ = media.Movie("Hot fuzz",
                       "https://upload.wikimedia.org/wikipedia/en/"
                       "thumb/c/c9/HotFuzzUKposter.jpg/"
                       "250px-HotFuzzUKposter.jpg",
                       "https://www.youtube.com/watch?v=ayTnvVpj9t4",
                       "Edgar Wright",
                       "Simon Pegg, Nick Frost, Martin Freeman",
                       "2007", False, None)

IDIOCRACY = media.Movie("Idiocracy",
                        "https://upload.wikimedia.org/wikipedia/en/"
                        "6/6b/Idiocracy_movie_poster.jpg",
                        "https://www.youtube.com/watch?v=BBvIweCIgwk",
                        "Mike Judge",
                        "Luke Wilson, Maya Rudolph, Dax Shepard",
                        "2006", False, None)

IN_A_WORLD = media.Movie("In a World",
                         "https://upload.wikimedia.org/wikipedia/en/"
                         "thumb/4/49/In_a_World_poster.jpg/"
                         "220px-In_a_World_poster.jpg",
                         "https://www.youtube.com/watch?v=bZHBjLFu5is",
                         "Lake Bell",
                         "Lake Bell, Fred Melamed, Michaela Watkins",
                         "2013", False, None)

LIFE_AQUATIC = media.Movie("Life Aquatic",
                           "https://upload.wikimedia.org/wikipedia/en/"
                           "thumb/7/7c/Lifeaquaticposter.jpg/"
                           "220px-Lifeaquaticposter.jpg",
                           "https://www.youtube.com/watch?v=yh401Rmkq0o",
                           "Wes Anderson",
                           "Bill Murray, Owen Wilson, Anjelica Huston",
                           "2004", True, None)

LOST_IN_TRANSLATION = media.Movie("Lost in Translation",
                                  "https://upload.wikimedia.org/wikipedia/en/"
                                  "thumb/4/4c/Lost_in_Translation_poster.jpg/"
                                  "220px-Lost_in_Translation_poster.jpg",
                                  "https://www.youtube.com/"
                                  "watch?v=yYAS92XPvIM",
                                  "Sofia Coppola",
                                  "Bill Murray, Scarlett Johansson,\
                                   Giovanni Ribisi ",
                                  "2003", False, None)

METROPOLIS = media.Movie("Metropolis",
                         "https://upload.wikimedia.org/wikipedia/en/"
                         "thumb/0/06/Metropolisposter.jpg/"
                         "220px-Metropolisposter.jpg",
                         "https://www.youtube.com/watch?v=ZSExdX0tds4",
                         "Fritz Lang",
                         "Brigitte Helm, Alfred Abel, Gustav Frohlich",
                         "1927", False, None)

MOONRISE_KINGDOM = media.Movie("Moonrise Kingdom",
                               "https://upload.wikimedia.org/wikipedia/en/"
                               "thumb/4/4f/Moonrise_Kingdom_FilmPoster.jpeg/"
                               "220px-Moonrise_Kingdom_FilmPoster.jpeg",
                               "https://www.youtube.com/watch?v=7N8wkVA4_8s",
                               "Wes Anderson",
                               "Jared Gilman, Kara Hayward, Bruce Willis",
                               "2012", True, None)

OFFICE_SPACE = media.Movie("Office Space",
                           "https://upload.wikimedia.org/wikipedia/en/"
                           "thumb/8/8e/Office_space_poster.jpg/"
                           "220px-Office_space_poster.jpg",
                           "https://www.youtube.com/watch?v=_IwzZYRejZQ",
                           "Mike Judge",
                           "Ron Livingston, Jennifer Aniston, David Herman",
                           "1999", False, None)

PI = media.Movie("Pi",
                 "https://upload.wikimedia.org/wikipedia/en/"
                 "thumb/5/5a/Piposter.jpg/"
                 "220px-Piposter.jpg",
                 "https://www.youtube.com/watch?v=jo18VIoR2xU",
                 "Darren Aronofsky",
                 "Sean Gullette, Mark Margolis, Ben Shenkman",
                 "1998", False, None)

PULP_FICTION = media.Movie("Pulp Fiction",
                           "https://upload.wikimedia.org/wikipedia/en/"
                           "thumb/8/82/Pulp_Fiction_cover.jpg/"
                           "220px-Pulp_Fiction_cover.jpg",
                           "https://www.youtube.com/watch?v=s7EdQ4FqbhY",
                           "Quentin Tarantino",
                           "John Travolta, Uma Thurman, Samuel L. Jackson",
                           "1994", False, None)

ROYAL_TENENBAUMS = media.Movie("Royal Tenenbaums",
                               "https://upload.wikimedia.org/wikipedia/en/"
                               "thumb/3/3b/The_Tenenbaums.jpg/"
                               "220px-The_Tenenbaums.jpg",
                               "https://www.youtube.com/watch?v=HaMfV72q40U",
                               "Wes Anderson",
                               "Gene Hackman, Gwyneth Paltrow,\
                                Anjelica Huston",
                               "2001", True, None)

RUSHMORE = media.Movie("Rushmore",
                       "https://upload.wikimedia.org/wikipedia/en/"
                       "thumb/4/42/Rushmoreposter.png/"
                       "220px-Rushmoreposter.png",
                       "https://www.youtube.com/watch?v=GxCNDpvGyss",
                       "Wes Anderson",
                       "Jason Schwartzman, Bill Murray, Olivia Williams",
                       "1998", True, None)

SECRETARY = media.Movie("Secretary",
                        "https://upload.wikimedia.org/wikipedia/en/"
                        "thumb/b/b9/Secretarymovpost.jpg/"
                        "220px-Secretarymovpost.jpg",
                        "https://www.youtube.com/watch?v=AFma24S-Uvw",
                        "Steven Shainberg",
                        "James Spader, Maggie Gyllenhaal, Jeremy Davies",
                        "2002", False, None)


SHAUN_OF_THE_DEAD = media.Movie("Shaun of the Dead",
                                "https://upload.wikimedia.org/wikipedia/en/"
                                "thumb/e/ec/Shaun-of-the-dead.jpg/"
                                "220px-Shaun-of-the-dead.jpg",
                                "https://www.youtube.com/watch?v=z-lmF5DAssU",
                                "Edgar Wright",
                                "Simon Pegg, Nick Frost, Kate Ashfield",
                                "2004", False, None)

SHOPGIRL = media.Movie("Shopgirl",
                       "https://upload.wikimedia.org/wikipedia/en/"
                       "thumb/5/57/Shopgirl.jpg/"
                       "220px-Shopgirl.jpg",
                       "https://www.youtube.com/watch?v=Bz4KxyvaaXM",
                       "Anand Tucker",
                       "Steve Martin, Claire Danes, Jason Schwartzman",
                       "2005", False, None)


SOCIAL_NETWORK = media.Movie("Social Network",
                             "https://upload.wikimedia.org/wikipedia/en/"
                             "thumb/7/7a/Social_network_film_poster.jpg/"
                             "220px-Social_network_film_poster.jpg",
                             "https://www.youtube.com/watch?v=lB95KLmpLR4",
                             "David Fincher",
                             "Jesse Eisenberg, Andrew Garfield,\
                              Justin Timberlake",
                             "2010", False, None)

TRUE_ROMANCE = media.Movie("True Romance",
                           "https://upload.wikimedia.org/wikipedia/en/"
                           "thumb/d/d6/True_romance.jpg/"
                           "215px-True_romance.jpg",
                           "https://www.youtube.com/watch?v=_wNYNDzKpuQ",
                           "Tony Scott",
                           "Christian Slater, Patricia Arquette,\
                            Dennis Hopper",
                           "1993", False, None)

VIRGIN_SUICIDES = media.Movie("Virgin Suicides",
                              "https://upload.wikimedia.org/wikipedia/en/"
                              "thumb/c/c9/Virgicides.jpg/"
                              "220px-Virgicides.jpg",
                              "https://www.youtube.com/watch?v=n8LCLCUJYd8",
                              "Sofia Coppola",
                              "Kirsten Dunst, Josh Hartnett, James Woods",
                              "1999", False, None)

movies = [ADVENTURELAND, ALMOST_FAMOUS, AMERICAN_HUSTLE, AMERICAN_PSYCHO,
          BIG_LEBOWSKI, BIRDMAN, BROKEN_FLOWERS, DARJEELING_LIMITED,
          EDWARD_SCISSORHANDS, EYES_WIDE_SHUT, GHOST_WORLD, GOODFELLAS,
          GRAND_BUDAPEST_HOTEL, HOLY_MOUNTAIN, HOT_FUZZ, IDIOCRACY,
          IN_A_WORLD, LIFE_AQUATIC, LOST_IN_TRANSLATION, METROPOLIS,
          MOONRISE_KINGDOM, OFFICE_SPACE, PI, PULP_FICTION, ROYAL_TENENBAUMS,
          RUSHMORE, SECRETARY, SHAUN_OF_THE_DEAD, SOCIAL_NETWORK, TRUE_ROMANCE,
          VIRGIN_SUICIDES]

fresh_tomatoes.open_movies_page(movies)
